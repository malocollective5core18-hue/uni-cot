...
from customers.models import TenantSubscription, create_owner_tenant, get_base_domain
...
def login_view(request, *args, **kwargs):
    ...
    if request.method == 'POST':
        ...
        if '@' in login_identifier:
            ...
            owner = OwnerUser.objects.filter(email__iexact=login_identifier, is_active=True).first()
            ...
            if member and check_password(password, member.password):
                # Resolve member's tenant (may be None for owners without tenant)
                member_tenant = getattr(member.owner, 'tenant', None)

                # If a tenant context exists in the request, ensure the member
                # belongs to that tenant. This preserves the previous safety
                # checks when logging in from a tenant domain.
                if tenant and member_tenant and member_tenant.id != tenant.id:
                    messages.error(request, 'This member account belongs to a different domain.')
                    return _tenant_redirect(request, 'service:welcome')

                # Verify member exists in owner core records when required
...
def register_view(request, *args, **kwargs):
    ...
    if request.method == 'POST':
        ...
        if tenant and getattr(tenant, 'schema_name', None) != 'public':
            ...
            if existing_member:
                existing_member_tenant = getattr(existing_member.owner, 'tenant', None)
...
def api_owner_admin_session(request, *args, **kwargs):
    ...
    try:
        owner = OwnerUser.objects.get(id=owner_id, is_active=True)
    except OwnerUser.DoesNotExist:
        return JsonResponse({'success': True, 'authenticated': False})
...
def api_owner_vault(request, *args, **kwargs):
    ...
    owner = OwnerUser.objects.filter(id=owner_id, is_active=True).first()
    if not owner:
        _clear_service_session(request)
