...
def api_owner_admin_login(request, *args, **kwargs):
    ...
    email = data.get('email', '').strip()
    password = data.get('password', '')
    tenant_slug = data.get('tenant_slug', '').strip()  # Make optional

    if not email or not password:
        return _json_error('Email and password are required')

    # Get the current tenant from request context, session, or provided slug
    tenant = _get_tenant_from_request(request)
    
    # If no tenant from request/session, try to get by slug
    if not tenant and tenant_slug:
        from customers.models import CRTenant
        tenant = CRTenant.objects.filter(subdomain=tenant_slug, is_active=True).first()
    
    # If still no tenant, try to find tenant by owner email (primary fallback)
    if not tenant:
        try:
            owner_check = OwnerUser.objects.select_related('tenant').get(email__iexact=email, is_active=True)
            if owner_check.tenant and owner_check.tenant.is_active:
                tenant = owner_check.tenant
        except OwnerUser.DoesNotExist:
            pass
    
    if not tenant:
        return _json_error('No tenant found for this owner account. Please ensure your account has an active tenant.', status=400)
    
    if tenant:
        if not tenant.owner:
            return _json_error('This tenant has no owner configured.', status=400)
        if tenant.owner.email.lower() != email.lower():
            return _json_error('Invalid email for this tenant. Only the tenant owner can access admin.', status=401)
        if not check_password(password, tenant.owner.password):
            return _json_error('Invalid password.', status=401)
        if not tenant.owner.is_active:
            return _json_error('Owner account is not active.', status=401)
        owner = tenant.owner
    else:
        owner = OwnerUser.objects.filter(email__iexact=email, is_active=True).first()
