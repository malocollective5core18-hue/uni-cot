...
from core.models import SystemSetting, User as CoreUser, UserGroup, UserGroupMember
...
def _ensure_owner_core_user(owner, reg_number, full_name=None, email=None):
    reg_number = (reg_number or '').strip()
    if not owner or not reg_number:
        return None

    defaults = {
        'full_name': (full_name or reg_number).strip(),
        'created_by': owner.id,
        'is_active': True,
        'is_verified': True,
        'role': 'member',
    ...
    core_user, created = CoreUser.objects.get_or_create(
        created_by=owner.id,
        registration_number=reg_number,
        defaults=defaults,
    )
    ...
    changed_fields = []
    if not core_user.is_active:
        core_user.is_active = True
        changed_fields.append('is_active')
    if not core_user.is_verified:
        core_user.is_verified = True
        changed_fields.append('is_verified')
    if core_user.role != 'member':
        core_user.role = 'member'
        changed_fields.append('role')
    if changed_fields and not created:
        core_user.save(update_fields=changed_fields)

    return core_user


def _ensure_tenant_owner_row(public_owner):
    """
    Tenant-local service tables have foreign keys to tenant-local owner_users.
    Keep the tenant row id aligned with the public owner id so existing session
    payloads and public tenant ownership checks keep working.
    """
    if not public_owner:
        return None

    local_owner = OwnerUser.objects.filter(id=public_owner.id).first()
    if local_owner:
        changed_fields = []
        for field in ('email', 'program_name', 'password', 'is_owner', 'is_active', 'phone_number'):
            public_value = getattr(public_owner, field)
            if getattr(local_owner, field) != public_value:
                setattr(local_owner, field, public_value)
                changed_fields.append(field)
        if changed_fields:
            changed_fields.append('updated_at')
            local_owner.save(update_fields=changed_fields)
        return local_owner

    return OwnerUser.objects.create(
        id=public_owner.id,
        email=public_owner.email,
        program_name=public_owner.program_name,
        phone_number=public_owner.phone_number,
        password=public_owner.password,
        is_owner=public_owner.is_owner,
        is_active=public_owner.is_active,
    )
...
def _cache_key_for_welcome(tenant, user):
    tenant_key = tenant.id if tenant else 'public'
    user_key = user.get('owner_id') if user and user.get('user_type') == 'owner' else 'guest'
    return f"welcome_page:{tenant_key}:{user_key}"


def _clear_welcome_cache(tenant, owner=None):
    if not tenant and not owner:
        return

    if not owner and tenant:
        owner = _get_tenant_owner(tenant)

    # Owner-specific cache entry for the tenant welcome page
    if owner:
        cache.delete(_cache_key_for_welcome(tenant, {'user_type': 'owner', 'owner_id': owner.id}))

    # Guest/anonymous welcome page cache key may also be stale
    cache.delete(_cache_key_for_welcome(tenant, {}))


def _cache_key_for_owner_dashboard(owner):
    return f"owner_dashboard:{owner.id}" if owner else 'owner_dashboard:public'


def _clear_owner_dashboard_cache(owner):
    if owner:
        cache.delete(_cache_key_for_owner_dashboard(owner))


def _cache_key_for_member_dashboard(member):
    return f"member_dashboard:{member.id}" if member else 'member_dashboard:public'


def _clear_member_dashboard_cache(member):
    if member:
        cache.delete(_cache_key_for_member_dashboard(member))


def _cache_key_for_system_demo(tenant):
    return f"system_demo:{tenant.id}" if tenant else 'system_demo:public'


def _clear_system_demo_cache(tenant):
    if tenant:
        cache.delete(_cache_key_for_system_demo(tenant))
...
def _get_owner_session_context(request):
    ...
    return owner, _ensure_tenant_owner_row(owner)


def _find_member_login_record(login_identifier, password, tenant=None):
...
def _find_active_owner_for_program(program_name):
    ...
    with schema_context(getattr(settings, 'PUBLIC_SCHEMA_NAME', 'public')):
        return (
            ...
            .filter(program_name__iexact=program_name, is_active=True)
            .first()
        )
...
def welcome(request, *args, **kwargs):
    ...
    review_summary = {
        'total_reviews': 0,
        'average_rating': 0,
        'average_rating_display': '0.0',
        'rating_bars': [{'rating': r, 'count': 0, 'percentage': 0} for r in range(5, 0, -1)],
        'star_states': ['empty'] * 5,
    }
    ...
    if owner:
        cached = cache.get(_cache_key_for_welcome(tenant, {'user_type': 'owner', 'owner_id': owner.id}))
        if cached is None:
            approved_comments_qs = Comment.objects.filter(owner=owner, status='approved').prefetch_related('replies', 'member').order_by('-created_at')
            comments = list(approved_comments_qs[:WELCOME_PAGE_COMMENT_LIMIT])
            review_summary = _build_review_summary(approved_comments_qs)
            ...
            cache.set(_cache_key_for_welcome(tenant, {'user_type': 'owner', 'owner_id': owner.id}), cached, int(os.getenv('DJANGO_CACHE_TIMEOUT', '300')))
        else:
            comments = cached['comments']
            review_summary = cached['review_summary']
            comment_count = cached['comment_count']
            pending_comments = cached['pending_comments']
...
def system_demo(request, *args, **kwargs):
    ...
    tenant = _get_tenant_from_request(request)
    tenant_owner = _get_tenant_owner(tenant) if tenant and getattr(tenant, 'schema_name', None) != 'public' else None
    system_program_name = (
        tenant_owner.program_name
        if tenant_owner
        else (tenant.name if tenant and getattr(tenant, 'name', None) else 'UNI-COT')
    )
    ...
    return render(request, 'system_index.html', {
        'tenant': tenant,
        'tenant_base_path': _tenant_base_path(request=request, tenant=tenant),
        'system_program_name': system_program_name,
...
def login_view(request, *args, **kwargs):
    ...
    if request.method == 'POST':
        ...
        if '@' in login_identifier:
            ...
            member_record = _find_member_login_record(login_identifier, password, tenant=_get_tenant_from_request(request))
            if member_record:
...
def register_view(request, *args, **kwargs):
    ...
    Public host: member signup by program name and email.
...
def owner_dashboard(request, *args, **kwargs):
    ...
    if request.method == 'POST':
        ...
        if action == 'save_contact_number':
            ...
            if not phone_number:
                ...
                with schema_context(getattr(settings, 'PUBLIC_SCHEMA_NAME', 'public')):
                    OwnerUser.objects.filter(id=owner.id).update(
    ...
    cache_key = _cache_key_for_owner_dashboard(owner)
    dashboard_cache = cache.get(cache_key)

    members = Member.objects.filter(owner=owner)
    comments = Comment.objects.filter(owner=owner).select_related('member').order_by('-created_at')
    ...
    if dashboard_cache is None:
        dashboard_cache = {
            ...
            'approved_count': approved_comments.count(),
            'rejected_count': rejected_comments.count(),
        }
        cache.set(cache_key, dashboard_cache, int(os.getenv('DJANGO_CACHE_TIMEOUT', '300')))
    ...
    return render(request, 'owner_dashboard.html', {
        ...
        'member_count': dashboard_cache['member_count'],
        'tenant': tenant,
...
def member_dashboard(request, *args, **kwargs):
    ...
    comments = Comment.objects.filter(owner=owner, status='approved').select_related('member').order_by('-created_at')
    
    return render(request, 'member_dashboard.html', {
        'member': member,
        'comments': comments,
        'member_program_owner_label': tenant.name or owner.program_name or member.program_name,
        'tenant_base_path': _tenant_base_path(request=request, tenant=tenant),
    })
...
def add_comment(request, *args, **kwargs):
    ...
    _clear_welcome_cache(_get_tenant_from_request(request), owner=member.owner)
    _clear_owner_dashboard_cache(member.owner)
    _clear_system_demo_cache(_get_tenant_from_request(request))
...
def react_comment(request, comment_id, reaction, *args, **kwargs):
    ...
    _clear_welcome_cache(_get_tenant_from_request(request), owner=comment.owner)
    _clear_owner_dashboard_cache(comment.owner)
    _clear_system_demo_cache(_get_tenant_from_request(request))
...
def register_member(request, *args, **kwargs):
    ...
    with transaction.atomic():
        _ensure_owner_core_user(tenant_owner, reg_number)
...
@require_http_methods(["POST"])
def update_member(request, member_id, *args, **kwargs):
    """Update member identity/status and optionally reset password."""
    owner, tenant_owner = _get_owner_session_context(request)
    if not owner or not tenant_owner:
        messages.error(request, 'Only owners can update members.')
        return _tenant_redirect(request, 'service:welcome')

    member = Member.objects.filter(id=member_id, owner=tenant_owner).first()
    if not member:
        messages.error(request, 'Member not found.')
        return _tenant_redirect(request, 'service:owner_dashboard')

    old_reg_number = member.reg_number
    reg_number = request.POST.get('reg_number', '').strip()
    program_name = request.POST.get('program_name', '').strip()
    ...
    if not reg_number or not program_name:
        messages.error(request, 'Registration number and program are required.')
        return _tenant_redirect(request, 'service:owner_dashboard')

    if Member.objects.filter(reg_number=reg_number).exclude(id=member.id).exists():
        messages.error(request, 'Registration number already exists.')
        return _tenant_redirect(request, 'service:owner_dashboard')

    if CoreUser.objects.filter(
        created_by=tenant_owner.id,
        registration_number=reg_number,
    ).exclude(registration_number=old_reg_number).exists():
        messages.error(request, 'Registration number already exists in your main system records.')
        return _tenant_redirect(request, 'service:owner_dashboard')
    ...
    with transaction.atomic():
        member.reg_number = reg_number
        member.program_name = program_name
        member.is_active = is_active
        update_fields = ['reg_number', 'program_name', 'is_active', 'updated_at']
        if password:
        ...
        core_user = CoreUser.objects.filter(
            created_by=tenant_owner.id,
            registration_number=old_reg_number,
        ).first()
        if core_user:
            core_user.registration_number = reg_number
            core_user.is_active = is_active
            core_user.is_verified = True
            core_user.role = 'member'
            core_user.save(update_fields=['registration_number', 'is_active', 'is_verified', 'role', 'updated_at'])
        else:
            _ensure_owner_core_user(tenant_owner, reg_number)

    messages.success(request, f'Member {reg_number} updated successfully.')
    return _tenant_redirect(request, 'service:owner_dashboard')


@require_http_methods(["POST"])
def delete_member(request, member_id, *args, **kwargs):
    """Delete a member account owned by the current owner."""
    owner, tenant_owner = _get_owner_session_context(request)
    if not owner or not tenant_owner:
        messages.error(request, 'Only owners can delete members.')
        return _tenant_redirect(request, 'service:welcome')
    ...
    with transaction.atomic():
        ...
        if core_user_ids:
            group_ids = list(
                UserGroupMember.objects
                .filter(user_id__in=core_user_ids)
                .values_list('group_id', flat=True)
                .distinct()
            )
            UserGroupMember.objects.filter(user_id__in=core_user_ids).delete()
            for group_id in group_ids:
                UserGroup.objects.filter(id=group_id).update(
                    current_members=UserGroupMember.objects.filter(group_id=group_id, status='active').count()
                )
            CoreUser.objects.filter(id__in=core_user_ids).delete()
        member.delete()

    messages.success(request, f'Member {reg_number} deleted successfully.')
    return _tenant_redirect(request, 'service:owner_dashboard')
...
def moderate_comment(request, comment_id, action, *args, **kwargs):
    ...
    if action == 'approve':
        ...
        owner = comment.owner
        comment.delete()
        _clear_welcome_cache(_get_tenant_from_request(request), owner=owner)
        _clear_owner_dashboard_cache(owner)
        _clear_system_demo_cache(_get_tenant_from_request(request))
