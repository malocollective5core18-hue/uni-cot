...
def api_owner_admin_login(request, *args, **kwargs):
    ...
    Allows login from any domain, not just the tenant domain.
    """
    ...
    # Allow owner login from any device/domain - look up by email only
    from customers.models import CRTenant
    
    # Find the owner and their tenant
    owner = None
    tenant = None
    
    try:
        # First try to get tenant from request context
        tenant = _get_tenant_from_request(request)
        if tenant and getattr(tenant, 'owner', None):
            owner = tenant.owner
            # Verify email matches this tenant's owner
            if owner.email.lower() != email.lower():
                # Email doesn't match this tenant, search globally
                owner = CRTenant.objects.filter(owner__email__iexact=email).first()
                if owner:
                    owner = owner.owner
                    tenant = owner.tenant if hasattr(owner, 'tenant') else None
        else:
            # No tenant context, search globally for the owner
            owner_tenant = CRTenant.objects.filter(owner__email__iexact=email).first()
            if owner_tenant:
                owner = owner_tenant.owner
                tenant = owner_tenant
    except Exception:
        pass
    
    # Fallback: if still no owner, check legacy OwnerUser table
    if not owner:
        try:
