...
def _tenant_route_kwargs(request=None, tenant=None):
    ...
    if tenant and getattr(tenant, 'id', None) and getattr(tenant, 'subdomain', None):
        # Ensure tenant_key exists; try to extract from request.path, else generate
        if not getattr(tenant, 'tenant_key', None):
            if request is not None:
                import re
                m = re.match(r'^/t/[^/]+/\d+/([^/]+)/', request.path)
                if m:
                    tenant.tenant_key = m.group(1)
                else:
