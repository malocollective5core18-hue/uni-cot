...
def register_view(request, *args, **kwargs):
    ...
    if request.method == 'POST':
        ...
        with schema_context(tenant.schema_name):
            tenant_owner = _ensure_tenant_owner_row(owner)
            if Member.objects.filter(reg_number__iexact=email).exists():
                messages.error(request, 'This email already has a member account. Please log in.')
                return _tenant_redirect(request, 'service:welcome')

            CoreUser.objects.get_or_create(
                registration_number=email,
                defaults={
                    'full_name': email.split('@')[0] or email,
                    'email': email,
