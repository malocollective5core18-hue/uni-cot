...
def api_owner_admin_login(request, *args, **kwargs):
    ...
    Sets proper Django session and rotates CSRF token.
    """
    ...
    # Step 3: Check if specified tenant exists (case-insensitive)
    from customers.models import CRTenant
    ...
    # Step 5: Establish Django session (protect against session fixation)
    request.session.flush()
    request.session.cycle_key()
    request.session['service_user'] = {
        'user_type': 'owner',
    ...
    # Step 6: Rotate CSRF token for security
    rotate_token(request)
    new_csrf_token = get_token(request)

    # Step 7: Return success with new CSRF token
    return JsonResponse({
        'success': True,
