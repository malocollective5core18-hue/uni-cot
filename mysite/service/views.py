...
from customers.models import create_owner_tenant, get_base_domain, _generate_unique_tenant_key
...
def _tenant_route_kwargs(request=None, tenant=None):
    ...
    if not tenant and request is not None:
        ...
        if session_tenant_id:
            ...
            tenant = CRTenant.objects.filter(id=session_tenant_id, is_active=True).first()
    ...
    # Ensure tenant_key exists
    tenant_key = getattr(tenant, 'tenant_key', None)
    if not tenant_key:
        if request is not None:
            import re
            m = re.match(r'^/t/[^/]+/\d+/([^/]+)/', request.path)
            if m:
                tenant_key = m.group(1)
        if not tenant_key:
        ...
        try:
            ...
            pass  # Silently continue with generated key
...
def api_owner_admin_login(request, *args, **kwargs):
    ...
    Establishes Django session and rotates CSRF token."""
    ...
    # Get tenant from request path (set by TenantMiddleware)
    tenant = _get_tenant_from_request(request)
    ...
    # Establish Django session (protect against session fixation)
    request.session.flush()
    request.session.cycle_key()
    request.session['service_user'] = {
        'user_type': 'owner',
    ...
    # Rotate CSRF token and return new token
