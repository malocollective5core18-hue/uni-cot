...
class CrossDeviceLoginTests(TestCase):
    ...
    def test_django_admin_can_log_in_from_welcome_page_to_founder_dashboard(self):
        ...
        response = self.client.post(
            reverse("service:login"),
            {
                "login_identifier": founder.email,
                "password": "secret123",
            },
            follow=False,
        )
        ...
        self.assertEqual(response.status_code, 302)
        self.assertTrue(response.url.endswith("/founder/saas-control/"), response.url)
...
class MemberRegistrationPersistenceTests(TestCase):
    def test_api_create_member_creates_missing_main_registry_record(self):
        ...
        response = self.client.post(
            reverse("service:api_create_member"),
            data='{"reg_number":"BCIT-001","program_name":"BCIT","password":"member123"}',
            content_type="application/json",
        )

        self.assertEqual(response.status_code, 200)
        self.assertTrue(Member.objects.filter(owner=owner, reg_number="BCIT-001").exists())
        self.assertTrue(
...
class OwnerMemberManagementTests(TestCase):
    ...
    def test_owner_can_update_member_and_reset_password(self):
        ...
        response = self.client.post(
            reverse("service:update_member", args=[member.id]),
            {
                "reg_number": "BCIT-002",
                "program_name": "BCIT Updated",
                "status": "inactive",
                "password": "newpass",
        ...
        self.assertEqual(member.reg_number, "BCIT-002")
        self.assertEqual(member.program_name, "BCIT Updated")
        self.assertFalse(member.is_active)
        self.assertTrue(check_password("newpass", member.password))
        self.assertTrue(
            CoreUser.objects.filter(
                created_by=self.owner.id,
                registration_number="BCIT-002",
                is_active=False,
                is_verified=True,
            ).exists()
        )

    def test_owner_can_delete_member_and_related_registry_records(self):
        member = Member.objects.create(
            owner=self.owner,
            reg_number="BCIT-001",
            program_name="BCIT",
        ...
        UserGroupMember.objects.create(user_id=core_user.id, group_id=group.id, status="active")

        response = self.client.post(reverse("service:delete_member", args=[member.id]), follow=False)

        self.assertEqual(response.status_code, 302)
        self.assertFalse(Member.objects.filter(id=member.id).exists())
        self.assertFalse(CoreUser.objects.filter(id=core_user.id).exists())
        self.assertFalse(UserGroupMember.objects.filter(user_id=core_user.id).exists())
