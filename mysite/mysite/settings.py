...
# Do not auto-load arbitrary root env files in production.
env_path = BASE_DIR / '.env'
if env_path.exists():
    load_dotenv(dotenv_path=env_path)

# Temporary guard: ensure this module is loaded as the inner settings module
# and not through an unexpected package path.
if __name__ != 'mysite.mysite.settings' or __package__ != 'mysite.mysite':
    raise ImproperlyConfigured(
        "Inner settings module loaded incorrectly: "
        f"__name__={__name__!r}, __package__={__package__!r}. "
        "Expected import from mysite/settings.py using "
...
if DATABASE_URL:
    ...
    parsed_db_config.setdefault('OPTIONS', {})
    parsed_db_config['OPTIONS'].setdefault('options', '-c client_encoding=UTF8')
    parsed_db_config['OPTIONS'].setdefault('sslmode', os.getenv('POSTGRES_SSLMODE', 'require'))
...
if REDIS_URL:
    ...
    CACHES = {
        'default': {
            ...
            'LOCATION': os.getenv('DJANGO_CACHE_LOCATION', 'ring0-api-cache'),
            'TIMEOUT': int(os.getenv('DJANGO_CACHE_TIMEOUT', '300')),
        }
    }
...
REALTIME_PUBLIC_ENABLED = os.getenv('REALTIME_PUBLIC_ENABLED', 'false').strip().lower() in {'1', 'true', 'yes', 'on'}
REALTIME_PUBLIC_MAX_SOCKETS_PER_IP = int(os.getenv('REALTIME_PUBLIC_MAX_SOCKETS_PER_IP', '3'))
REALTIME_PUBLIC_MAX_SOCKETS_PER_TENANT = int(os.getenv('REALTIME_PUBLIC_MAX_SOCKETS_PER_TENANT', '50'))
REALTIME_PUBLIC_MAX_SOCKETS_TOTAL = int(os.getenv('REALTIME_PUBLIC_MAX_SOCKETS_TOTAL', '100'))
REALTIME_PUBLIC_CONNECTION_LIMIT = int(os.getenv('REALTIME_PUBLIC_CONNECTION_LIMIT', '20'))
